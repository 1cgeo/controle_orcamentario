...
Versão 1.1 (2026-06-18). Modelo conceitual: entidades, relacionamentos e regras. O DDL exato (fonte da verdade) vive em `er/*.sql`, aplicado por `create_config.js`. Este documento dá a visão de alto nível; ao mudar o schema, mude `er/` e atualize aqui.
...
- **valor_nc** é o recebido e nunca muda por devolução; a devolução/anulação de empenho vive em `nota_empenho.valor_anulado`. O **`valor_recolhido`** da NC é a parte do crédito recebido que foi devolvida/recolhida (informada na própria NC pelo usuário); é **informativo** e não desconta de `valor_nc` (o recebido segue cheio). Na 3.1 ele agrega por ND (split PDR/Extra-PDR, mesmo recorte do recebido por `data_emissao`); na 3.2/3.7 aparece como coluna "Valor Recolhido" da NC.
