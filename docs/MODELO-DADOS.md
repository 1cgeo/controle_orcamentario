...
`code` + `nome` (e campos extras), populados no seed de `er/dominio.sql`:
- **natureza_despesa** (`code` VARCHAR PK, `nome`, `gnd`, `grupo`): 10 NDs usadas pela DGEO (339014/339015 diárias, 339030 material, 339033 passagens, 339039 serviços, 339040 TIC, 339047 obrigações, 339139 publicações, 449040 TIC capital, 449052 permanente). GND 3 = custeio, 4 = capital.
...
- **tipo_licitacao** (1 GCALC DSG, 2 Própria), **classificacao_nc** (1 PDR, 2 Extra-PDR), **tipo_item_dfd** (1 Material, 2 Servico), **grau_prioridade** (1 Alta, 2 Normal, 3 Baixa), **tipo_posto_grad** (postos/graduações, para `dgeo.usuario`).
...
- NC que mistura duas ND no mesmo número: duas linhas com o mesmo `numero` e ND distinta (sem UNIQUE em `numero`).
